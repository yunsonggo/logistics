      ...
      >
        ...
        <el-form-item label="描述" prop="desc">
          <el-input
            ...
            :placeholder="giveInfo.desc"
            @input="change($event)"
          >
          </el-input>
        </el-form-item>
        <el-form-item>
          <el-button type="primary" @click="handleGiveEditForm"
            >立即提交</el-button
          >
          <el-button @click="giveEditFormReset">取消</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
import { reqGiveEdit } from "../../../api/index";
...
export default {
  ...
  beforeRouteEnter(to, from, next) {
    next((vm) => {
      vm.giveInfo = to.params.givesInfo;
      vm.giveInfo.state == 1 ? (vm.giveState = true) : (vm.giveState = false);
      console.log(vm.giveInfo);
    });
  },
  methods: {
    // 多层嵌套无法输入解决方法
    ...
    handleGiveEditForm() {
      this.$delete(this.giveInfo, "shop_ids");
      this.$delete(this.giveInfo, "is_top");
      ...
      this.editGive(manager_token, data)
    },
    giveEditFormReset() {
      this.$router.go(-1);
    },
    async editGive(token,data) {
      const result = await reqGiveEdit(token, data);
      if (result.code === 0) {
        this.$message({
          message: "编辑服务信息成功",
          type: "success",
