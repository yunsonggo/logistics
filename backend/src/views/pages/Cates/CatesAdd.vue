  ...
  <div style="width: 100%">
    ...
    <el-card style="margin-top: 20px; width: 600px">
      ...
      >
        ...
        <el-form-item>
          <el-button type="primary" @click="handleCateAddForm"
            >立即提交</el-button
          >
          <el-button @click="cateAddFormReset">取消</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
import {reqCategoryAdd,reqCategroy} from "../../../api/index"
export default {
  name: "catesadd",
  data() {
    return {
  ...
  methods: {
    ...
    handleCateAddForm() {
      ...
      this.addCategory(manager_token,data);
    },
    cateAddFormReset() {
      this.$router.go(-1);
    },
    async addCategory(manager_token,data) {
        const result = await reqCategoryAdd(manager_token,data)
        if (result.code === 0) {
        this.$message({
          message: "添加类别信息成功",
          type: "success",
