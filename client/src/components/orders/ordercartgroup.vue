<template>
    <section class="checkout-section cart-group">
        <h3>{{orderInfo.shop_name}}</h3>
        <ul>
            <li v-for="(item,index) in orderInfo.sale_goods_list" :key="index">
                <img :src="static_url + item.icon" alt="">
                <div class="cart-group-info">
                    <span>{{item.good_name}}</span>
                    <span>x {{item.good_count}}</span>
                    <span>{{item.good_price.toFixed(2)}}</span>
                </div>
            </li>
            <li class="cart-group-total">
                <div>小计</div>
                <div>¥{{orderInfo.goods_price.toFixed(2)}}</div>
                <!-- <div v-else>¥ {{showDeliveryPrice}}</div> -->
            </li>
            <li class="cart-group-total">
                <div>配送费</div>
                <div>¥{{delPrice.toFixed(2)}}</div>
                <!-- <div v-else>¥ {{showDeliveryPrice}}</div> -->
            </li>
            <li class="cart-group-total">
                <div class="discounts">
